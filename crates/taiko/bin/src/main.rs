...
use reth_optimism_cli::{chainspec::TaikoChainSpecParser, Cli};
use reth_optimism_node::{args::RollupArgs, TaikoNode};
...
fn main() {
    ...
    if let Err(err) =
        Cli::<TaikoChainSpecParser, RollupArgs>::parse().run(|builder, rollup_args| async move {
            ...
            match use_legacy_engine {
                false => {
                    ...
                    let handle = builder
                        ...
                        .with_components(TaikoNode::components(rollup_args.clone()))
                        .with_add_ons(TaikoNode::new(rollup_args).add_ons())
                ...
                true => {
                    let handle =
                        builder.node(TaikoNode::new(rollup_args.clone())).launch().await?;
