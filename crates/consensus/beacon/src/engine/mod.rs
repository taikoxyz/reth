...
type PendingForkchoiceUpdate<PayloadAttributes> =
    (ForkchoiceState, Option<PayloadAttributes>, oneshot::Sender<RethResult<OnForkChoiceUpdated>>, EngineApiMessageVersion, bool);
        ...
        match make_canonical_result {
            Ok(outcome) => {
                    ...
                    self.ensure_consistent_forkchoice_state(state)?
                {
                    ...
                    self.process_payload_attributes(attrs, head, state, version)
            ...
            Ok(on_updated_result) => {
                if let Some(on_updated) = on_updated_result {
                    ...
                    let replaced_pending =
                        self.pending_forkchoice_update.replace((state, attrs, tx, version, debug));
                    warn!(
                        target: "consensus::engine",
                        hook = %hook.name(),
                    ...
                    self.set_blockchain_tree_action(
                        BlockchainTreeAction::MakeForkchoiceHeadCanonical { state, attrs, tx, version, debug },
                    );
                }
            }
            Err(error) => {
    ...
    fn on_blockchain_tree_action(
        ...
        match action {
            BlockchainTreeAction::MakeForkchoiceHeadCanonical { state, attrs, tx, version, debug } => {
                ...
                match self
                    .on_forkchoice_updated_make_canonical_result(state, attrs, result, elapsed, version, debug)
                {
                    Ok(on_updated) => {
        ...
        'main: loop {
            ...
            loop {
                ...
                if this.hooks.active_db_write_hook().is_none() {
                    if let Some((state, attrs, tx, version, debug)) = this.pending_forkchoice_update.take() {
                ...
                if let Poll::Ready(Some(msg)) = this.engine_message_stream.poll_next_unpin(cx) {
                    match msg {
                        BeaconEngineMessage::ForkchoiceUpdated { state, payload_attrs, tx, version, debug } => {
                            this.on_forkchoice_updated(state, payload_attrs, tx, version, debug);
                        }
